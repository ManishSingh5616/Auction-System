    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#f0ede6", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: "600px", margin: "3rem auto", padding: "0 2rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: "3rem" }}>
          <div style={{ width: "80px", height: "80px", borderRadius: "50%", background: "#1e1e1e", border: "2px solid #e8c547", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.5rem", fontWeight: 800, fontFamily: "'Syne', sans-serif", marginBottom: "1rem", color: "#f0ede6" }}>
            {user?.name?.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2)}
          </div>
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.5rem", fontWeight: 800, margin: "0 0 0.25rem" }}>{user?.name}</h1>
          <span style={{ background: "#e8c54722", color: "#e8c547", padding: "0.2rem 0.75rem", borderRadius: "100px", fontSize: "0.75rem", fontWeight: 600 }}>🏷️ Bidder</span>
        </div>

        {/* Info */}
        <div style={{ background: "#141414", borderRadius: "16px", padding: "1.5rem 2rem", border: "1px solid #1e1e1e" }}>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "1rem", fontWeight: 700, margin: "0 0 0.5rem", letterSpacing: "-0.01em" }}>Account Info</h2>
          {row("Name", user?.name)}
        ...
        <p style={{ color: "#333", fontSize: "0.8rem", textAlign: "center", marginTop: "2rem" }}>
